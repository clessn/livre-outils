...
### La période moderne : Un premier âge d’or (DATE)
...
### L’ère informatique : de nouvelles possibilités (DATE)
...
L’une des contributions majeures à la visualisation moderne est la formalisation d’une *grammaire des graphiques*. Contrairement à une approche qui catégorise les visualisations par « types » (diagramme en barres, nuage de points, etc.), une grammaire vise à décomposer tout graphique en un ensemble de composants fondamentaux. Wilkinson, Anand et Grossman [@wilkinson_etal05] proposent ainsi une structure théorique permettant de décrire systématiquement les éléments constitutifs d’un graphique : données, variables esthétiques, transformations statistiques, objets géométriques, échelles, systèmes de coordonnées et guides. Cette formalisation a deux avantages principaux. Sur le plan conceptuel, elle permet de comprendre les liens entre des visualisations en apparence différentes mais reposant sur les mêmes principes sous-jacents. Sur le plan pratique, elle ouvre la voie à des outils plus souples, capables de générer une variété quasi infinie de graphiques à partir d’un petit nombre de règles combinatoires.  

C’est dans cet esprit qu’Hadley Wickham a développé **ggplot2**, une implémentation en R de cette grammaire, qu’il affine dans sa version dite *layered grammar of graphics* [@wickham10]. Plutôt que de voir un graphique comme une entité figée, ggplot2 le conçoit comme une superposition de *couches* (*layers*). Chaque couche combine un jeu de données, une transformation statistique (par exemple un lissage ou un regroupement en classes), un objet géométrique (points, lignes, barres, etc.) et un ajustement de position. Cette approche modulaire rend possible la construction incrémentale de graphiques : on peut partir d’un nuage de points, puis ajouter un lissage, des annotations ou encore des facettes pour comparer des sous-groupes. Les échelles, coordonnées et guides (axes, légendes) complètent l’architecture et assurent une cohérence visuelle.  
