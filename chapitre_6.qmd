...
La section suivante présente la méthode d'utilisation de l'outil *rvest*. Elle démontre comment l'outil permet d'extraire des données du web, une méthode en vogue en science sociales.
...
Il est important de noter que ce chapitre ne permet que d'extraire des documents html uniques. Afin d'extraire plusieurs page web simultanément, il faudra utiliser d'autres package ainsi que des boucles, ce qui est trop complexe pour cet ouvrage d'introduction. Maintenant que nous savons extraire des informations d'un document html pour le mettre dans une base de données, voici d'autres applications pratiques de rvest à cet effet.
...
Bien qu'un seul chapitre ne permet pas de relever l'ensemble des outils de collecte de données disponibles pour la communauté scientifique, les outils présentés permettent un aperçu à la fois d'outils plus conventionnels et répandus de collecte de données numériques, mais également de dresser un portrait du potentiel d'extraction permis par la maîtrise de R.
