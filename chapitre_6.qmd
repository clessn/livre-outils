...
En somme, les extracteurs webs, et dans ce cas précis le package rvest, sont des outils qui facilitent grandement l'acquisition de données massives. Plutôt que d'avoir à payer pour des données dont la qualité n'est pas assurée, l'utilisation d'extracteurs permet un accès plus facile et précis à des données provenant de sites web. L'utilisation de ces outils peut toutefois s'avérer complexe et nécessitent un certain nombre de connaissances en lien avec les langages de programmation. Le @sec-chap2 offre un survol du langage fonctionnel R, qui est utilisé par de nombreux développeurs lors de l'écriture d'extracteurs et qui permet d'utiliser le package *rvest*. R est également reconnu pour ces fonctionnalités statistiques qui sont, elles aussi, abordées ultérieurement dans ce livre. L'utilisation de cet outil permet aux chercheurs de faire plusieurs coups d'une seule pierre. Non seulement *rvest* permet de se familiariser avec le langage R, ce qui est un atout essentiel dans la recherche en sciences sociales numériques, mais ce même développement permet un accès inégalé à des données massives qui pourront ensuite être analysées. Ainsi, des connaissances en R sont un atout fondamental au développement d'extracteurs, et la complexité du code évoluera dépendamment du site qui sera extrait.

On peut rajouter deux défis que l'on peut rencontrer lors de l'usage des extracteurs webs outre la nécessité des connaissances en codage : la gestion des sites web complexes et la détection d'IP (Internet Protocol). Selon le site Octoparse, on estime que 20% des sites webs sont difficiles à scraper. Ce n'est pas un pourcentage élevé, mais ce n'est pas négligeable. Il existe de nombreux outils qui permettent l'extraction des données de sites web simples. Or, à l'heure actuelle, il y a une augmentation de sites Web qui intègrent des techniques avancées comme AJAX (Asynchronous Javascript And XML) qui permet la mise à jour du contenu d'une page Web avec des requêtes HTTP en arrière-plan sans avoir à recharger complètement la page (site Web MDN Web Docs, 2025). Cela requiert donc des outils de scraping plus fonctionnels (Octoparse, 2023). Pour ce qui est de la détection d'IP, ça peut être un défi dans la mesure où l'IP peut être bloqué par le site web cible. Cela est dû au fait que : «[l]e scraping intensif d’un site Web entraîne un trafic important, qui peut surcharger un serveur Web et causer des pertes économiques au propriétaire du site.» (Octoparse, 2023) Heureusement, de nombreuses astuces existent pour éviter ce blocage et certains outils de web scraping intègrent déjà la fonctionnalité qui permet de résoudre le problème.
