...
Cette méthode, avec le logiciel $\textsf{R}$, via l'outil *rvest* permet de récolter des données qui pourront ensuite être traitées et exploiter avec $\textsf{R}$, ce qui favorise donc la compatibilité de l'outil. De plus, les données extraites peuvent être exportées dans de nombreux formats tels que des fichiers .csv, .RDS, .xlsx, et plus encore.
...
La section suivante présente la méthode d'utilisation de l'outil *rvest*. Elle démontre comment l'outil permet d'extraire des données du web, une méthode en vogue en sciences sociales.
...
Pour débuter l'extraction des données, il suffit de copier l'URL de la page web à extraire et la coller dans l'appel de la fonction `read_html()`. Il est important de stocker l'URL dans l'objet `html_LEGISinfo`.
...
D'abord, `html_elements` doit être utilisé en premier pour trouver toutes les observations souhaitées, car cette fonction retourne une liste de tous les nœuds qui correspondent à l'appel de la fonction. Le nombre d'observations est indiqué par `xml_nodeset()`. Comme `html_element` retourne seulement le premier élément qui correspond, il faut l'utiliser en deuxième, après `html_elements`. Cette seconde fonction à pour but de trouver les éléments qui deviendront les variables à extraire. Pour l'exemple de LEGISinfo, nous commencerons par extraire tous les éléments \\\<a\\\>. Comme `html_elements` retourne une liste, nous voulons commencer avec cette fonction.
...
Il est important de noter que ce chapitre ne permet que d'extraire des documents HTML uniques. Afin d'extraire plusieurs pages web simultanément, il faudra utiliser d'autres packages ainsi que des boucles, ce qui est trop complexe pour cet ouvrage d'introduction. Maintenant que nous savons extraire des informations d'un document HTML pour le mettre dans une base de données, voici d'autres applications pratiques de rvest à cet effet.
...
En revanche, ce chapitre s'ancre autour du postulat qu'il ne faut pas se limiter aux outils de récolte de données traditionnels, et que la révolution numérique engendre d'importantes opportunités d'acquisition de données massives et exclusives par le biais des extracteurs de données, plus précisément l'outil d'extraction *rvest*. Ces extracteurs ont pour but de récolter des données issues d'un site web afin de les rendre disponibles pour l'analyse scientifique.
