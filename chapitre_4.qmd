...
![](images/chapitre_4_toolbox_padlock_black.png){fig-align="center" width="12%"}
...
[^chapitre_4-1]: https://science.gc.ca/site/science/fr/science-ouverte
...
Ce mouvement s'est concrétisé par des politiques à tous les niveaux. À l'échelle internationale, 194 pays ont adopté à l'UNESCO en 2021 la *Recommandation sur une science ouverte*[^chapitre_4-2], un cadre mondial soulignant l'importance de rendre la science plus accessible et transparente pour tous. Au Canada, la conseillère scientifique en chef, Mona Nemer, a publié la *Feuille de route pour la science ouverte*[^chapitre_4-3] en 2020, suivie en 2021 par la Politique des trois organismes sur la gestion des données de recherche[^chapitre_4-4] (CRSNG, IRSC et CRSH). Au Québec, les Fonds de recherche du Québec (FRQ) se sont également dotés d'une ambitieuse Politique de diffusion en libre accès[^chapitre_4-5] en 2019, mise à jour en 2022.

[^chapitre_4-2]: https://unesdoc.unesco.org/ark:/48223/pf0000379949_fre

[^chapitre_4-3]: https://www.canada.ca/fr/innovation-sciences-developpement-economique/nouvelles/2020/02/feuille-de-route-pour-la-science-ouverte-visant-a-reduire-les-obstacles-et-a-accelerer-les-decouvertes.html

[^chapitre_4-4]: https://science.gc.ca/site/science/fr/financement-interorganismes-recherche/politiques-lignes-directrices/gestion-donnees-recherche/politique-trois-organismes-gestion-donnees-recherche

[^chapitre_4-5]: https://frq.gouv.qc.ca/science-ouverte/politique-diffusion-2022/
...
Des outils comme *Open Science Framework* (*OSF*)[^chapitre_4-6] offrent une plateforme complète pour la recherche ouverte, permettant de gérer l'ensemble d'un projet de recherche, incluant la préinscription.

[^chapitre_4-6]: https://help.osf.io/article/330-welcome-to-registrations
...
Outre *OSF*, de multiples plateformes offrent l'enregistrement des hypothèses, comme *AsPredicted*[^chapitre_4-7] ou le *Center for Open Science*[^chapitre_4-8].

[^chapitre_4-7]: https://aspredicted.org/

[^chapitre_4-8]: https://www.cos.io/initiatives/prereg

Au-delà de la préinscription des hypothèses, plusieurs organismes subventionnaires exigent maintenant la rédaction d'un plan de gestion des données (PGD) dès la demande de financement. Ce document décrit comment les données seront collectées, organisées, documentées, entreposées et éventuellement partagées tout au long du projet. Au Canada, l'Assistant PGD[^chapitre_4-9] est l'outil officiel développé par l'Alliance de recherche numérique pour guider les chercheurs dans cette démarche. Il propose des modèles adaptés aux exigences des trois organismes fédéraux (CRSH, CRSNG, IRSC) et de plusieurs autres institutions.

[^chapitre_4-9]: https://dmp-pgd.ca/

### Collecte et analyse

Avant de choisir où entreposer ses données, un principe s'impose : *ne jamais conserver de fichiers uniquement sur son ordinateur local*. Un disque dur peut tomber en panne, un ordinateur peut être volé ou perdu. Tout fichier qui n'existe qu'en local est un fichier à risque. Dès le début d'un projet, chaque document, chaque script, chaque donnée devrait être synchronisé sur un service infonuagique ou versionné sur *GitHub* (idéalement les deux, selon la nature des fichiers).
...
La question centrale ici n'est pas seulement « où entreposer ? », mais surtout *« où peut-on entreposer les données selon leur nature ? »* Car toutes les solutions ne se valent pas, et certaines sont inappropriées selon le type de données manipulées.
...
#### Organiser son cloud comme on organise son bureau

Un cloud mal structuré n'est guère mieux qu'un bureau encombré. Pour que les chemins de fichiers restent stables et que la collaboration soit fluide, il vaut la peine de réfléchir dès le départ à une arborescence logique et cohérente. Une structure simple pourrait ressembler à ceci :
...
Cette organisation présente deux avantages majeurs. D'abord, les chemins de fichiers dans les scripts d'analyse restent prévisibles et stables d'un collaborateur à l'autre. Ensuite, elle facilite la distinction entre ce qui appartient au cloud (données, documents) et ce qui appartient à un dépôt `Git` (code, scripts), ces deux espaces devant rester séparés.
...
tbl_outils |>
  ...
  cols_label(
    Critères = "Critères",
    `*OSF*` = md("*OSF*"),
    `*GitHub*/*GitLab*` = md("*GitHub/ GitLab*"),
    `Solutions institutionnelles` = "Institutionnelles",
    `Solutions grand public` = "Grand public"
  ) |>
  tab_footnote(
    footnote = md("Ex. : *OneDrive*, *Nextcloud*"),
    locations = cells_column_labels(columns = `Solutions institutionnelles`)
  ) |>
  tab_footnote(
    footnote = md("Ex. : *Dropbox*, *Google Drive*"),
...
Bien qu'il existe des alternatives comme *GitLab* (déjà mentionné pour sa version à code source ouvert), `Git` et *GitHub* dominent largement le domaine grâce à leur communauté active et à leur adoption généralisée en milieu académique. Leur apprentissage représente un défi initial, mais les bénéfices en termes de traçabilité, de collaboration et de reproductibilité en font des outils essentiels pour le chercheur moderne.
...
**Important** : Il ne faut pas initialiser un dépôt `Git` à l'intérieur d'un dossier synchronisé par un service infonuagique comme *Dropbox* ou *OneDrive*. Ces services tentent de synchroniser les fichiers en continu, ce qui entre en conflit avec les opérations de `Git` et génère des erreurs et des doublons. Le dossier du projet doit donc vivre *en dehors* du cloud. C'est `Git` lui-même (via *GitHub*) qui assure la sauvegarde et le partage du code.
...
Le fichier `README.md` est la carte de visite du projet. Rédigé en `Markdown`, il accueille quiconque visite le dépôt sur *GitHub* et décrit en quelques lignes l'objectif du projet, son organisation et comment l'utiliser. C'est la première chose qu'un collaborateur ou un lecteur consultera.
...
On ajoute explicitement les dossiers et fichiers voulus pour qu'ils soient prêts à être validés.

``` bash
git add code/ documentation/ README.md
```

Il est préférable de nommer les fichiers ou dossiers plutôt que d'utiliser `git add -A`, qui ajoute tout le contenu du répertoire (risqué si le `.gitignore` est incomplet). Pour les débutants, `git add -A` reste cependant la solution simple qui répond le plus souvent au besoin lorsque le `.gitignore` est bien configuré.
...
Lorsque des collègues ont soumis une demande de fusion (*pull request*), on l'examine et on l'accepte directement sur le site *GitHub* via l'interface web. Une fois la fusion effectuée sur *GitHub*, on met à jour sa copie locale pour intégrer les changements :
...
Alors que le terminal reste une approche fondamentale pour maîtriser `Git` et *GitHub*, il existe des outils conviviaux tels que **`GitHub Desktop`** qui offrent une alternative intuitive. Cet outil simplifie le processus de gestion de versions décentralisée, en particulier pour ceux qui souhaitent commencer par une approche visuelle. La plupart des environnements de code comme `RStudio` et `VS Code` intègrent aussi `Git` directement dans leur interface. On peut y valider, créer des branches et pousser ses modifications sans quitter son éditeur, et `VS Code` propose en plus des extensions populaires comme `GitLens` qui enrichissent la visualisation de l'historique et des changements.
...
Plus récemment, l'arrivée des intelligences artificielles génératives a encore abaissé la barrière d'entrée de `Git` et de *GitHub*. Ces assistants maîtrisent parfaitement les commandes `Git` et peuvent interagir directement avec *GitHub*, qu'ils opèrent eux-mêmes en ligne de commande ou par l'entremise de protocoles d'intégration comme le *Model Context Protocol* (MCP). Il devient ainsi possible de confier l'ensemble du processus à un agent tel que *Claude* ou *ChatGPT*, de la création d'une branche à la rédaction d'une demande de fusion détaillée, jusqu'à la fusion finale, sans avoir à mémoriser ni même à taper la moindre commande. Comme le rappelle le @sec-chap9, ces outils évoluent vite, mais ils rendent aujourd'hui la gestion de versions accessible à quiconque, même sans maîtrise technique préalable.
