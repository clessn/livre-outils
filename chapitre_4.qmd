...
`But principal` = c(
"Donner un aperçu général de la littérature",
"Trouver d’autres articles à partir de références clés",
"Explorer un sujet large et cartographier les connaissances",
"Synthétiser rigoureusement les études sur un sujet précis",
"Combiner quantitativement les résultats de plusieurs études"
)
)

tt(tbl_types_revues,
width = 0.75,
caption = "Résumé des principaux types de synthèse de la connaissance"
)
``` 
...
### Illustration concrète d’une revue systématique en sciences sociales avec Covidence

Pour illustrer concrètement et simplement le fonctionnement de Covidence dans un contexte de revue sytématique en science sociale, prenons l’exemple suivant. Admettons une étude portant sur l’effet des programmes d’électrification ruraux sur les ménages au Kenya ; l’équipe de chercheurs effectuant la recherche commence par importer 2000 références bibliographiques provenant de Web Of Science, Google Scholar et JSTOR. 

	Etape 1 : title and abstract screening
Les chercheurs définissent des critères par exemple : électrification, ménages ruraux, Afrique subsaharienne, évaluations empiriques, etc. Ils peuvent également décider de mettre des critères d’exclusion. 
Covidence applique ces critères en analysant les titres et résumés des 2000 textes puis, il retient 550 qui entrent dans ces critères et seront soumis à l’étape 2.
Notons que Covidence met en évidence les situations où, pour un même article, un codeur a sélectionné “inclure” alors qu’un autre a choisi “exclure” ou “incertain”. Ces conflits sont automatiquement listés dans la plateforme, ce qui permet à l’équipe de les regarder ensemble et de discuter pour se mettre d’accord.

	Etape 2 : full text review
Ici, les membres de l’équipe de recherche, passe à la lecture des articles précédemment retenus. Pour un même article, au moins deux d’entre les membres répondent « oui », « non », « peut-être » selon les critères prédéfinis. Covidence extrait ensuite les articles qui ont reçu « oui » de l’ensemble des lecteurs et exclut ceux avec « non ». Pour les articles avec des réponses différentes, Covidence les mets en évidence pour que les chercheurs puissent se mettre d’accord.
A la fin de cette étape, il reste 50 articles respectent pleinement tous les critères par consensus et passent à la dernière phase.

	Etape 3 : extraction 
L’équipe procède ensuite à l’extraction des informations essentielles des 90 articles sélectionnés (méthodes, données, indicateurs, résultats). Covidence centralise ces éléments et génère des tableaux qui facilitent la comparaison entre les études et la rédaction de la synthèse.

Cet exemple illustre comment Covidence aide l’équipe à appliquer ses critères de manière uniforme et à gérer efficacement le volume d’articles à analyser pour une revue systématique.
...
<!-- SUGGESTION : Ajouter des images sur les étapes à suivre pour exporter la collection Zotero au format BibLatex, mais cela implique des connaissances (l'exportation elle-même) que je n'ai pas encore, donc je propose seulement. -->
