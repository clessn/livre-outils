...
Ce mouvement s'est concrétisé par des politiques à tous les niveaux. À l'échelle internationale, les 193 États membres de l'UNESCO ont adopté en 2021 la *Recommandation sur une science ouverte* [@unesco21], un cadre mondial soulignant l'importance de rendre la science plus accessible et transparente pour tous. Au Canada, la conseillère scientifique en chef, Mona Nemer, a publié la *Feuille de route pour la science ouverte* en 2020 [@gouvernementducanada20], suivie en 2021 par la *Politique des trois organismes sur la gestion des données de recherche* (CRSNG, IRSC et CRSH) [@gouvernementducanada21]. Au Québec, les Fonds de recherche du Québec (FRQ) se sont également dotés d'une ambitieuse *Politique de diffusion en libre accès* en 2019, mise à jour en 2022 [@fondsderechercheduquebec22].
...
Des outils comme l'[*Open Science Framework*](https://osf.io) (*OSF*) offrent une plateforme complète pour la recherche ouverte, permettant de gérer l'ensemble d'un projet de recherche, incluant la préinscription.
...
Outre *OSF*, d'autres registres accueillent la préinscription, comme [*AsPredicted*](https://aspredicted.org), dont le formulaire court convient bien aux devis expérimentaux simples.
...
Alors que le terminal reste une approche fondamentale pour maîtriser `Git` et *GitHub*, il existe des outils conviviaux tels que `GitHub Desktop` qui offrent une alternative intuitive. Cet outil simplifie le processus de gestion de versions décentralisée, en particulier pour ceux qui souhaitent commencer par une approche visuelle. La plupart des environnements de code comme `Positron` et `VS Code` intègrent aussi `Git` directement dans leur interface. On peut y valider, créer des branches et pousser ses modifications sans quitter son éditeur, et `VS Code` propose en plus des extensions populaires comme `GitLens` qui enrichissent la visualisation de l'historique et des changements.
