...
Les langages de bas niveau, comme les langages assembleurs, se rapprochent du langage machine.
...
Les IA comme *ChatGPT* ou *Gemini* sont aussi très pertinentes dans l'apprentissage du code.
Elles permettent d'expliquer la logique d'un code ou d'une fonction, d'aider à comprendre pourquoi un code ne marche pas et même de trouver des formulations ou des fonctions inattendues pour coder quelque chose.
...
Ces logiciels se distinguent aussi des langages de programmation par leur interface graphique intuitive permettant de réaliser rapidement des analyses statistiques complexes.
...
Il en existe trois versions : `RStudio Desktop`, qui est une version gratuite pour usage sur les ordinateurs personnels, `RStudio Server`, une version web accessible via le navigateur, et `RStudio Workbench`, une version professionnelle avec fonctionnalités avancées pour les entreprises.
...
Parmi les outils les plus utiles, on retrouve notamment une fenêtre d'aide, de la documentation sur les différents *packages*, un navigateur de l'environnement de travail, une visionneuse de données, ainsi que la prise en charge de la coloration syntaxique.
...
Cette disposition comprend une console, un éditeur de code source à onglets pour organiser les fichiers d'un projet, un espace dédié à l'environnement de travail, et un volet permettant d'afficher des graphiques ou de consulter la documentation sur les différents *packages*.
...
`RStudio` inclut également nativement le contrôle de version `Git`, facilitant la collaboration et le suivi des modifications pour le travail en équipe (voir le Chapitre 4).
...
Il convient aussi de mentionner que *Posit* développe un second environnement de travail, `Positron`, qui prend en charge à la fois $\textsf{R}$ et `Python`.
`Positron` est construit sur `Code OSS`, la base de logiciel libre maintenue par *Microsoft* sur laquelle repose également `Visual Studio Code`.
Les licences des deux outils diffèrent toutefois.
Bien que gratuit et bâti sur une fondation libre, `Positron` est distribué sous la *Elastic License*, qui rend son code consultable mais en restreint la modification et la redistribution.
`RStudio`, lui, demeure un logiciel libre pleinement maintenu par l'entreprise et sa communauté.
Les deux restent d'excellents choix, selon la préférence de chacun.
...
Dans le coin inférieur gauche se trouvent la console et le terminal.
Dans la console, on peut interagir avec $\textsf{R}$ de la même manière que dans le volet principal, mais le code ne sera pas enregistré.
...
On retrouve également différentes communautés, telles que *R-Ladies*, qui mettent de l'avant la diversité des genres dans la communauté du langage de programmation.
