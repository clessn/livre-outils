...
Ces langages ont des règles beaucoup plus strictes que les langues traditionelles comme le français ou l’anglais.
...
C et Rust sont réputés pour ses performances dans des systèmes complexes.
...
Dans la section suivante, nous examinons comment choisir quel langage de programmation apprendre en fonction de vos objectifs, de votre contexte et des ressources disponibles.
...
Bien que la majorité des langages de programmation principaux peuvent réaliser une grande quantité de tâches, ils ont tous leurs spécialités et leurs avantages.
...
Julia peut être intéressant pour ceux qui souhaitent faire de la science des données et de l'apprentissage machine.
...
Python et R ont des communautés très actives en plus d'une grande quantité de documentation et d'innombrables tutoriels gratuits, permettant l'apprentissage autonome et efficace.
Les programmeurs débutants peuvent s'inscrire sur des sites tels que DataCamp, Codecademy et Coursera afin d'avoir de l'aide dans leur apprentissage.
D'autres éléments à garder en tête lors du choix d'un langage de programmation est la courbe d'apprentissage, la marché du travail dans son domaine et sa région, ainsi que la compatibilité avec ses contraintes techniques et organisationnelles.

En contraste aux langages de programmation précédemment mentionnés, dits en « source ouverte », il existe des logiciels à licenses comme SAS, STATA, Excel et SPSS.
...
Les logiciels à licenses peuvent être installés et utilisés rapidement et à faible coûts en termes d'apprentissage, contrairement aux langages de programmation qui exigent une maîtrise de la syntaxe et des concepts algorithmique.
Ces logiciels se distinguent des langages de programmation notamment part leurs interfaces graphiques intuitive permettant de réaliser des analyses statistiques complexes par simple clic.
...
Cette simplicité représente un gain de temps non négligeable.
...
Il est souvent dit que R est un langage de programmation développé par des statisticiens, pour des statisticiens.
Le langage de programmation R a plusieurs avantages qui font de lui un outil puissant et utile pour tout chercheur.
Un de ces avantages majeurs est qu'il est en source ouverte.
Cela signifie que le langage de programmation R est gratuit d'utilisation, est développé par les utilisateurs et non par des grandes entreprises -- ce qui lui procure une plus grande flexibilité -- et qu'il permet aux utilisateurs de créer leurs propres fonctions pour répondre à leurs besoins spécifiques.
Le caractère source ouverte du langage de programmation R est une de ses charactéristiques qui rassemble un grande quantité de ces utilisateurs.
Ce nombre d'utilisateurs est d'ailleurs en croyance importante depuis plusieurs années.
...
Elle s'étend à l'ensemble de l'écosystème R, notamment à l'accès la formation, à la documentation et aux différents fonctionnalités se retrouvant dans les packages.
Les packages sont des groupes de fonctionnalités et d'extensions utilisées par les usagers du langage de programmation R afin d'étendre les possiblités du langage et de réaliser des opérations plus complexes.
De nouveaux packages comportant notamment de nouvelles méthodes statistiques et de visualisation graphique sont souvent implémentées et améliorées.

Le langage de programmation R est particulièrement versatile pour effectuer des tâches notamment en recherche en sciences sociales numériques, notamment liées aux statistiques et à la visualisation graphique.
Il dispose d'un large éventail de fonctions avec plusieurs milliers de packages disponibles.
Cette richesse de l'écosystème signifie que pratiquement toutes les techniques et méthodes statistiques -- des plus classiques aux plus récentes -- sont disponibles et utilisables dans R.
...
Ils sont aussi souvent bâtis sur des fondations communes permettant également aux programmeurs plus avancés de personnalisé et d'adapter leur code aux besoins de leurs analyses.
...
Les logiciels à licences sont coûteux, rigides et l'ajout de fonctionnalités se fait par les développeurs internes à la compagnie.
...
Il existe un débat en sciences sociales numériques sur le langage de programmation le plus approprié à apprendre.
...
En effet, le développement de package se fait de manière décentralisée et toute personne sachant programmer en R peut collaborer à cette communauté.
...
Bien que le site a une politique de dépôt stricte et les packages doivent être suffisamment documentés, il n'y a pas d'autorité centrale, ce qui peut refroidir certains usagers.
...
Sa syntaxe est considérée par certains comme étant plus intuitive et proche du langage naturel, en plus de bénéficier d'une communauté plus vaste et diversifiée.
De plus, dans certains domaines, le marché de l'emploi offre plus d'opportunités pour Python.
Toutefois, il n'est pas aussi performant que R pour l'analyse et la visualisation de données.
Pour sa part, Julia est avantageux pour des calculs intensifs, de la modélisation mathématique, ou si la performance est critique.
En effet, Julia est conçu pour être aussi rapide que C ou Fortran tout en restant aussi simple que Python, et il a été créé dès le départ pour le calcul haute performance et la recherche scientifique moderne.
Toutefois, Julia reste moins populaire quee R, avec moins de packages spécialisés, surtout pour les analyses statistiques.
Bien que pertinent, le débat sur le choix d'un langage de programmation est infini.
Il n'y a pas de réponse absolue et objective à quel langage de programmation un chercheur en sciences sociales numériques devrait apprendre.
Le choix dépend des objectifs spécifiques propres à chacun.
Cependant, considérant les tâches à effectuer par des programmeurs en sciences sociales numériques, apprendre le langage de programmation R est un excellent choix.


```{r}
#| tbl-cap: Résumé des principaux outils de programmation pour l'analyse de données
#| echo: false
#| message: false
#| warning: false
...
tbl_resume_chap_prog <- tibble(
  ...
  `Logiciels sous licence (SAS, SPSS, STATA)` = c(
    "Non", "Modérée", "Élevée dans certains secteurs",
    "Bonne", "Faible", "Limitée"
  ),
  Python = c(
    "Oui", "Très élevée", "Très populaire",
    "Excellente", "Bonne", "Très flexible"
  ),
  Julia = c(
    "Oui", "En croissance", "Modérée",
    "Bonne", "Bonne", "Très flexible"
  ),
  R = c(
    "Oui", "Très élevée",
    "Très populaire en sciences sociales et statistiques",
    "Très bonne", "Très élevée", "Très flexible"
  )
)
...
tbl_resume_chap_prog |>
  ...
  cols_label(
    Critères = "Critères",
    `Logiciels sous licence (SAS, SPSS, STATA)` = md("Logiciels sous licence<br>(SAS, SPSS, STATA)"),
    Python = "Python",
    Julia = "Julia",
    R = "R"
  ) |>
  ...
  cols_width(
    Critères ~ pct(32),
    `Logiciels sous licence (SAS, SPSS, STATA)` ~ pct(17),
    Python ~ pct(17),
    Julia ~ pct(17),
    R ~ pct(17)
  ) |>
...
Ces logiciels à license ont joué un rôle crucial dans la standardisation de l’analyse statistique en sciences sociales, en offrant des plateformes robustes, mais souvent coûteuses et rigides.
...
Le créateurs de langage de programmation S -- et ceux de R par extension -- ne viennent pas d'un background de programmation classique.
En effet, l'objectif à la base de ces langages était de rendre plus facile et accessible l'analyse de données.
Le langage de programmation R a répondu à des besoins pressants de la communauté scientifique de l'époque : ceux de pouvoir accéder à des outils puissants sans avoir à payer des licences onéreuses et de bénéficier d’une liberté totale dans le développement de nouvelles méthodes d’analyse.
...
Des entreprises telles que RStudio (maintenant Posit) développent des outils intégrés qui rendent R plus accessible et qui permettent d'aller encore plus loin, poussant les limites d'un langage souvent perçu comme étant principalement utilisé pour l'analyse de données.
L'écosystème R se diversifie avec des packages comme ggplot2 (servant à la visualisation graphique), tidyverse (servant à la manipulation et au traitement de données), ou Shiny (qui permet de faire applications web interactives).
