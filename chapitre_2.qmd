...
\< Avant de parler de l'environnement IDE, on pourrait par exemple spécifier qu'il y'a plusieurs environements où l'on peut coder en R mais que chaque offre différents avantages dépendamment des besoins de chacun. Pour Coder R, vous pouvez utiliser un environnement de développement intégré (IDE). L'IDE centralise tous les aspects de la programmation, comme l'édition du code, la compilation et le déblocage en un seul endroit.
