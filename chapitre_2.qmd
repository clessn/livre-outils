...
Il convient aussi de mentionner que *Posit* développe un second environnement de travail, `Positron`, qui prend en charge à la fois $\textsf{R}$ et `Python`. `Positron` est construit sur `Code OSS`, la base de logiciel libre maintenue par *Microsoft* sur laquelle repose également `Visual Studio Code`. Les licences des deux outils diffèrent toutefois. Bien que gratuit et bâti sur une fondation libre, `Positron` est distribué sous la *Elastic License*, qui rend son code consultable mais en restreint la modification et la redistribution. `RStudio`, lui, demeure un logiciel libre pleinement maintenu par l'entreprise et sa communauté. Les deux restent d'excellents choix, selon la préférence de chacun.
