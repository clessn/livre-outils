# Outils de programmation : $\textsf{R}$ comme point de départ {#sec-chap2}
...
Les IA comme *ChatGPT* ou *Gemini* sont aussi très pertinentes dans l'apprentissage du code. Elles permettent d'expliquer la logique d'un code ou d'une fonction, d'aider à comprendre pourquoi notre code ne marche pas et même de trouver des formulations ou des fonctions inattendues pour coder quelque chose.
...
Les logiciels propriétaires peuvent être installés et utilisés rapidement, contrairement aux langages de programmation qui exigent une maîtrise de la syntaxe et des concepts algorithmiques.
...
Le langage de programmation $\textsf{R}$ présente des avantages uniques qui en font un choix pertinent pour la recherche en sciences sociales numériques spécifiquement, et la science des données plus généralement.
Le principal avantage est que le code source est ouvert.
Cela signifie que le langage de programmation $\textsf{R}$ est gratuit d'utilisation et développé par les utilisateurs et non par une grande entreprise. Cela lui procure une plus grande flexibilité et permet aux utilisateurs de modifier le langage pour répondre à leurs besoins spécifiques.
...
De nouveaux *packages* comportant notamment de nouvelles méthodes statistiques et de visualisations graphiques sont constamment ajoutés et améliorés. Ainsi, le développement du langage se fait de manière décentralisée et toute personne sachant programmer en $\textsf{R}$ peut collaborer à cette communauté.
...
Cette richesse de l'écosystème signifie que pratiquement toutes les techniques et méthodes statistiques, des plus classiques aux plus récentes, sont disponibles et utilisables dans $\textsf{R}$. Il est souvent dit que $\textsf{R}$ est un langage de programmation développé par des statisticiens, pour des statisticiens.
...
## Manuel d'instruction : apprendre à programmer en $\textsf{R}$
...
Comme mentionné précédemment, le langage de programmation $\textsf{R}$ est l'un des langages de statistiques et d'exploration de données les plus populaires en sciences sociales numériques.
Le langage de programmation $\textsf{R}$ est pris en charge par de nombreux environnements de programmation.
Plusieurs ont été spécialement conçus pour la programmation en $\textsf{R}$, le plus notable étant `RStudio`, tandis que d'autres sont des environnements de programmation universels, tels que `Visual Studio Code` qui prennent en charge $\textsf{R}$ via des extensions et des modules complémentaires.

### Qu'est-ce que RStudio ?

`RStudio` (maintenant développé par *Posit*) est un projet de logiciel libre destiné à regrouper les différentes composantes du langage de programmation $\textsf{R}$ en un seul outil.
`RStudio` fonctionne sur tous les systèmes d'exploitation, y compris *Windows*, *macOS* et *Linux*.
...
`RStudio` facilite l'utilisation du langage de programmation $\textsf{R}$ en offrant de nombreux outils permettant à l'utilisateur de réaliser aisément ses tâches.
Parmi les outils les plus utiles, on retrouve notamment une fenêtre d'aide, de la documentation sur les différents *packages* $\textsf{R}$, un navigateur de l'environnement de travail, une visionneuse de données, ainsi que la prise en charge de la coloration syntaxique.
...
Ce volet permet également d'accéder au répertoire des *packages* disponibles pour $\textsf{R}$ et à l'arborescence des fichiers de l'utilisateur.
...
Par exemple, `RStudio` excelle dans la création de documents `R Markdown` (ou `Quarto`), permettant de mélanger code, texte et résultats pour créer des rapports, présentations ou sites web dynamiques.
Il permet aussi d'utiliser des langages de balisage tels que `Markdown` et \LaTeX.
...
Il convient aussi de mentionner que *Posit* développe un second environnement de travail, `Positron`, qui prend en charge à la fois $\textsf{R}$ et `Python`. `Positron` est construit sur `Code OSS`, la base de logiciel libre maintenue par *Microsoft* sur laquelle repose également `Visual Studio Code`. Il s'agit toutefois d'un cas intéressant du point de vue des licences : bien que gratuit et bâti sur une fondation libre, `Positron` est lui-même distribué sous une licence (la *Elastic License*) qui rend son code consultable, mais en restreint la modification et la redistribution, contrairement à `RStudio`, qui demeure un logiciel libre pleinement maintenu par l'entreprise et son énorme communauté d'utilisateurs dévoués. Les deux demeurent d'excellents choix : le recours à l'un ou à l'autre relève avant tout de la préférence de chaque utilisateur.
...
Dans le coin inférieur gauche se trouvent la console et le terminal. Dans la console, on peut interagir avec $\textsf{R}$ de la même manière que dans le volet principal, mais le code ne sera pas enregistré.
...
L'environnement global est l'endroit où l'utilisateur peut voir les bases de données, les fonctions et les différents autres objets $\textsf{R}$ actifs.
Il peut cliquer sur les divers éléments actifs pour les consulter.
L'onglet historique permet à l'utilisateur de consulter les derniers morceaux de code $\textsf{R}$ qu'il a exécutés ainsi que les dernières commandes saisies dans la console.
...
Il est conçu pour fonctionner avec divers outils permettant de travailler avec des bases de données en $\textsf{R}$ dans `RStudio`.
...
L'onglet *Plots* permet de visualiser les graphiques générés à partir de $\textsf{R}$, que ce soit en utilisant `ggplot2`, `lattice` ou *base* $\textsf{R}$.
...
L'onglet *Help* permet à l'utilisateur de rechercher et de consulter de la documentation sur de nombreux sujets, notamment sur les différentes fonctions en $\textsf{R}$ ainsi que sur les *packages*.
...
Bien que le langage de programmation $\textsf{R}$ ne doive pas obligatoirement être utilisé avec `RStudio`, nous pensons que, pour la plupart des utilisateurs, leur utilisation conjointe est bénéfique.
`RStudio` permet également d'utiliser différents langages de balisage compatibles avec $\textsf{R}$, facilitant ainsi l'utilisation de plusieurs outils complémentaires.
...
Le langage de programmation $\textsf{R}$ est plus qu'un simple outil statistique, il est au centre d'une grande communauté de personnes qui ont à cœur des principes liés à l'inclusion et à l'avancement humain.
