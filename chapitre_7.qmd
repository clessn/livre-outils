...
Toutefois, le temps qu'ils peuvent prendre pour générer des graphiques diffère. Dans le cadre du développement d'une application, la compagnie Jumping Rivers, spécialisée dans la formation en science des données, s'est posé la question de savoir lequel entre les deux packages `ggplotly` et `plotly` serait le plus rapide pour générer un graphique interactif. Le résultat obtenu est qu'en moyenne `ggplotly` est environ 23 fois plus lent que `plotly` (Roe & Gillespie, 2017) Ainsi, les deux sont pratiques pour générer des graphiques interactifs, mais selon le temps que vous disposez, vous saurez lequel prioriser. 
...
C'est un package flexible car les applications `shiny` peuvent être déployées sur plusieurs plateformes comme les plateformes Cloud (AWS, Google Cloud, etc.) ou encore GitHub Pages afin d'héberger statiquement les applications simples. La réactivité de son système est également un atout car elle permet d'avoir une expérience fluide et interactive sans code complexe lors de son utilisation (Datanovia, 2025). 
