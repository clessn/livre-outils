...
## Baliser les sciences sociales&nbsp;: langages et pratiques {#sec-chap7}
...
```{=html}
À l'instar du tableau comparatif à la page 150 (chapitre 8), il serait bien de pouvoir retrouver un tableau des avantages et inconvénients des différents langages de balisages.Bien qu'on puisse imaginer le lecteur passer à travers le chapitre, on se doute qu'une fois qu'ils s'apprêtent à jouer dans les différents langages, il voudra quelque chose de plus synthétique à ce moment.
...
tt(tbl_langages_balisage, width =  c(.3, .2, .2, .2, .2), caption = 'Résumé des critères de sélection - Langages de balisage', notes = list(
