...
Les autres couches des graphiques *ggplot2* dépendent souvent du codeur et des étapes de construction de son graphique[^2] [@wickham2016ggplot2, p. 77 et 89-93]. Le reste de ce chapitre présente la grammaire de *ggplot2* avec un exemple de construction de graphique à bande présenté couche par couche.
...
[^1]: Plusieurs options disponibles comme des histogrammes avec la fonction *histogram()* ou des graphiques de densité avec la fonction *densityplot()*.

[^2]: Les étapes (couches) d'un graphique *ggplot2* ne sont pas nécessairement dans le même ordre d'un graphique à un autre.
