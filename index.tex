...
utilisateurs peuvent télécharger gratuitement, et dans lequel aucun
achat supplémentaire est nécessaire pour avoir accès à des
fonctionnalités additionnels. Bien que ce logiciel soit déjà riche en
...
pourrais résumer mon intérêt avec sous la notation suivante:
\(P(Y = Survie | X = Femme)\). En d'autres termes, la probabilité de
survie étant donné que nous soyons une femme. Pour ce faire, je dois
utiliser l'ensemble de données \texttt{Titanic}, disponible en format
csv. Je dois donc installer et télécharger la librairie \texttt{readr}
afin que \texttt{R} puisse importer et lire les données. Ensuite je vais
utiliser la commande \texttt{table}, inclue dans les commandes de base
de \texttt{R}, afin de produire un tableau croisé entre les variables
femme et survie.
...
\NormalTok{dat }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"data/titanic.csv"}\NormalTok{)}

\FunctionTok{table}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{survie, dat}\SpecialCharTok{$}\NormalTok{femme) }
\end{Highlighting}
\end{Shaded}
...
données à propos des passagers du Titanic. Toutefois, le format du
tableau n'est pas très esthétique. Pour remédier à ce problème, nous
pouvons installer et utiliser la librarie \texttt{modelsummary} qui nous
permettra de créer rapidement des tableaux croisés plus esthétique, et
qui contiendrons davantage d'informations, facilitant la lecture et
notre compréhension de la relation qui nous intéresse.
...
\NormalTok{Tableau\_2}\FloatTok{.1} \OtherTok{\textless{}{-}} \FunctionTok{datasummary\_crosstab}\NormalTok{(survie }\SpecialCharTok{\textasciitilde{}}\NormalTok{ femme, }\AttributeTok{data =}\NormalTok{ dat, }\AttributeTok{title =} \StringTok{\textquotesingle{} \textquotesingle{}}\NormalTok{) }

\NormalTok{Tableau\_2}\FloatTok{.1}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dat}\SpecialCharTok{$}\NormalTok{femme[dat}\SpecialCharTok{$}\NormalTok{femme }\SpecialCharTok{==} \DecValTok{0}\NormalTok{] }\OtherTok{\textless{}{-}} \StringTok{"Homme"}  
...
\FunctionTok{table}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{femme)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics{images/chapitre5_vscode.png}
...
{\centering \includegraphics{images/chapitre5_CVtemp.png}
...
{\centering \includegraphics{images/chapitre5_TStemp.png}
