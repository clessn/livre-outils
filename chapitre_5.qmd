...
COMMENTAIRE: Ici, j'ajouterais Litmaps, qui émerge comme l'acteur dominant dans le domaine considérant qu'il vient de racheter ResearchRabbit. Cela a modifié l'interface de ResearchRabbit et introduit une version payante. De plus, Litmaps est plus complet que Connected Papers. Je signale aussi qu'à mon avis, ResearchRabbit et Litmaps peuvent servir pour le snowballing, qui est mentionné ci-bas sans pour autant être associé à ces outils. En effet, les deux permettent de générer une carte à partir de plusieurs articles, puis de continuer (snowball) la construction de la cartographie en ajoutant des articles qui résultent de la recherche précédente à notre collection existante, et ainsi de suite (système de "trails").
