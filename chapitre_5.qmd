...
Le logiciel de gestion de références JabRef, bien que moins populaire que Mendeley et EndNote, mérite d'être mentionné ici pour plusieurs raisons. Premièrement, c'est un logiciel libre, gratuit et multiplateforme. JabRef utilise le format BibTeX, mais il peut aussi aider à gérer d'autres formats bibliographiques (INRAE, 2020, p.4). Deuxièmement, c'est un programme simple à utiliser et qui fonctionne sous différents systèmes d'exploitation : Windows, Linux, etc. Troisièmement, on peut paramétrer les formats d'affichage et d'exportation. La plupart des paramètres de ce logiciel sont également exportables (Toulin et al., 2009, p. 59).
Malgré ces avantages, JabRef n'est pas connu en sciences sociales car il est plus utilisé dans les domaines comme l'informatique, les mathématiques, etc. Par ailleurs, la connaissance de JAVA qui est requise si l'on veut réaliser des tâches plus complexes (Toulin et al., 2009, p. 62) peut décourager les utilisateurs. Ainsi, même si JabRef n'est pas très connu, il est pertinent de le connaître car c'est un logiciel open source qui est tout de même un critère important dans le choix d'un outil numérique de recherche.
