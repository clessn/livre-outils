...
La revue de littérature est souvent perçue comme une étape préliminaire. Elle constitue pourtant une contribution scientifique à part entière, à condition d'être conduite avec rigueur et méthode. Une telle démarche permet non seulement de cerner les zones de connaissances encore inexplorées, mais aussi de situer comment la recherche en cours s'inscrit dans un domaine circonscrit. Loin d'être une simple collection de sources choisies aléatoirement, la synthèse de la connaissance doit suivre une démarche méthodique et réfléchie, intégrant des critères tels que la pertinence, la crédibilité, la nouveauté sur le sujet, la diversité des perspectives, la qualité méthodologique, l'impact et la citation des travaux, et leur objectivité.

En dressant un portrait structuré et critique de la littérature existante, le chercheur établit les fondements de sa démarche scientifique et éclaire la formulation de sa question de recherche. Une revue de littérature rigoureuse n'est pas un simple recensement de sources ; c'est une contribution scientifique à part entière qui délimite le champ, identifie les lacunes et justifie chaque nouvelle étude. Cette démarche s'accompagne nécessairement d'une gestion rigoureuse des sources bibliographiques, et c'est là que les outils présentés dans ce chapitre entrent en jeu.

La citation des sources est une pratique incontournable dans le monde académique. Elle situe les travaux au sein d'un cadre scientifique établi, garantit la reproductibilité des analyses et assure une juste attribution du mérite aux contributions antérieures [@zaid_etal17; @hughes13; @racz_markovic18; @kostoff_cummings13]. Une référenciation rigoureuse est également ce qui distingue une démarche scientifique transparente d'une simple accumulation de lectures. C'est dans ce contexte que les outils de gestion des références prennent toute leur importance.

Pour aborder ces enjeux, ce chapitre défend une thèse centrale : la revue de littérature est une démarche scientifique à part entière, qui exige une méthodologie transparente et reproductible. Chercher des sources « au hasard » ne suffit pas : une revue rigoureuse doit documenter comment les études ont été identifiées, sélectionnées et analysées. Des méthodes comme le snowballing, la revue de portée ou la revue systématique répondent à cette exigence : elles sont publiables, reconnues et enseignées dans les universités. Si de nouveaux outils fondés sur l'intelligence artificielle (comme *Elicit*, *NotebookLM* ou *Scholarcy*) facilitent la découverte et la compréhension rapide de la littérature, ils ne remplacent pas une méthodologie solide. Ce chapitre ne vise pas à enseigner comment conduire une revue systématique (les bibliothèques universitaires offrent cet accompagnement), mais à en illustrer l’importance, à présenter les méthodes qui existent, et à introduire les outils qui aident à les appliquer.
...
Avant d'entrer dans les outils, il convient de glisser quelques mots sur les méthodes pour lesquelles ces outils sont utiles. Dans le cadre de l'élaboration d'une recherche scientifique, la phase de synthèse de la connaissance constitue une étape fondamentale permettant de circonscrire le champ d'étude, d'identifier les travaux antérieurs pertinents et de déceler les lacunes dans les connaissances existantes (@fig-cycle-recherche). Parmi les différentes approches méthodologiques adoptées pour mener à bien cette synthèse des connaissances, la revue narrative est généralement la plus populaire, notamment pour des travaux scolaires ou de courtes recherches. Des outils numériques, notamment ceux fondés sur l'intelligence artificielle, sont d'ailleurs très utiles dans ce contexte, car ils facilitent l'élargissement de la portée de la revue de littérature en proposant des références supplémentaires ou des publications liées aux travaux déjà identifiés.
...
Pour installer Better BibTeX, il faut accéder au menu des modules complémentaires (via le menu `Outils` sur Windows/Linux ou le menu `*Zotero*` sur macOS, puis `Modules complémentaires`), sélectionner l'option Installer un module à partir d'un fichier et choisir le fichier `.xpi` préalablement téléchargé. Une fois l'installation terminée, il convient de configurer le module via les préférences de *Zotero* (menu `Édition > Préférences` sur Windows/Linux ou `*Zotero* > Préférences` sur macOS) dans l'onglet Better BibTeX.

Afin d'assurer une cohérence dans les clés de citation utilisées au sein d'un groupe de travail, Better BibTeX permet de définir un format de clé de citation uniforme. Le format recommandé est fondé sur le nom de l'auteur et l'année de publication, par exemple : `authEtal2.fold.lower.replace(find=".",replace=_) + len + shortyear | veryshorttitle + shortyear`. Cette uniformisation garantit une gestion efficace des citations dans les documents collaboratifs.

### Génération du fichier .bib

Une fois Better BibTeX configuré, il est possible d'exporter une collection *Zotero* au format BibLaTeX. Pour cela, il suffit de faire un clic droit sur la collection souhaitée et de sélectionner Exporter la collection. Il faut choisir le format `Better BibLaTeX` et cocher l'option `[x] Keep updated`. Ce fichier `.bib` sera mis à jour automatiquement chaque fois que de nouvelles références seront ajoutées à *Zotero*. Cette méthode est particulièrement utile pour synchroniser les références dans des projets partagés via des systèmes de versionnement comme *GitHub*.
